<?php
  ...
  var dataString = 'page_id_cons='+ page_id_cons;
    ...
    $requete=" SELECT * FROM dossier_exp WHERE id_dossier!='' AND etat_dossier=0 ";
...
*/
	...
	if($last_page > 1){
        ...
        if($page > 1)
            $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(1);'>&laquo; Debut</a>";
        else
            $pagination.= "<span class='disabled'>&laquo; Debut</span>";
		
		if ($page > 1)
            $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(".($prev).");'>&laquo; Precedent&nbsp;&nbsp;</a>";
        else
        ...
        {   
            for ($counter = 1; $counter <= $last_page; $counter++)
            {
                if ($counter == $page)
                    ...
                    $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(".($counter).");'>$counter</a>";     
        ...
        elseif($last_page > 5 + ($adjacents * 2))
        {
            if($page < 1 + ($adjacents * 2))
            {
                ...
                $pagination.= "...";
                $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(".($second_last).");'> $second_last</a>";
                $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(".($last_page).");'>$last_page</a>";   
           
           }
           elseif($last_page - ($adjacents * 2) > $page && $page > ($adjacents * 2))
           {
               $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(1);'>1</a>";
               $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(2);'>2</a>";
               $pagination.= "...";
        ...
        if($page < $counter - 1)
            $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(".($next).");'>Suivant &raquo;</a>";
        else
            $pagination.= "<span class='disabled'>Suivant &raquo;</span>";
		
		if($page < $last_page)
            $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_actif(".($last_page).");'>Fin &raquo;</a>";
        else
    ...
    $req=" SELECT * FROM dossier_exp WHERE id_dossier!='' AND etat_dossier=0 ";
                                                            ...
                                                            <div class="card shadow-none border">
                                                                <div class="card-body p-3">
                                                                    <div>
                                                                        ...
                                                                        <div>
                                                                            <h5 class="font-size-14 text-truncate"><a href="javascript:void();" class="text-body">'.$row['num_dossier'].'</a></h5>
                                                                            <div class="mt-3">
