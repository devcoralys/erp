<?php
  ...
  var dataString = 'page_id_cons='+ page_id_cons;
    ...
    $requete=" SELECT * FROM dossier_exp WHERE id_dossier!='' AND type_transit_id=1 AND export=1 AND import=0 ";

/*
    if($_SESSION['is_dg']==1 || $_SESSION['is_valid']==1)
    {
        $requete=" SELECT * FROM dossier_expLEFT JOIN utilisateur ON utilisateur.secur=dossier.demandeur_secur WHERE id_dossier!='' ";
    }
*/
	...
	if($last_page > 1){
        ...
        if($page > 1)
            $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(1);'>&laquo; Debut</a>";
        else
            $pagination.= "<span class='disabled'>&laquo; Debut</span>";
		
		if ($page > 1)
            $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(".($prev).");'>&laquo; Precedent&nbsp;&nbsp;</a>";
        else
        ...
        {   
            for ($counter = 1; $counter <= $last_page; $counter++)
            {
                if ($counter == $page)
                    ...
                    $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(".($counter).");'>$counter</a>";     
        ...
        elseif($last_page > 5 + ($adjacents * 2))
        {
            if($page < 1 + ($adjacents * 2))
            {
                ...
                $pagination.= "...";
                $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(".($second_last).");'> $second_last</a>";
                $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(".($last_page).");'>$last_page</a>";   
           
           }
           elseif($last_page - ($adjacents * 2) > $page && $page > ($adjacents * 2))
           {
               $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(1);'>1</a>";
               $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(2);'>2</a>";
               $pagination.= "...";
        ...
        if($page < $counter - 1)
            $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(".($next).");'>Suivant &raquo;</a>";
        else
            $pagination.= "<span class='disabled'>Suivant &raquo;</span>";
		
		if($page < $last_page)
            $pagination.= "<a href='javascript:void(0);' onClick='change_page_dossier_maritime(".($last_page).");'>Fin &raquo;</a>";
        else
    ...
    $req=" SELECT * FROM dossier_exp WHERE id_dossier!='' AND type_transit_id=1 AND export=1 AND import=0 ";
 
    /*
    if($_SESSION['is_dg']==1 || $_SESSION['is_valid']==1)
    {
        $req=" SELECT * FROM dossier_expLEFT JOIN utilisateur ON utilisateur.secur=dossier.demandeur_secur WHERE id_dossier!='' ";
    }
    */
                                                            ...
                                                            <div class="card shadow-none border">
                                                                <div class="card-body p-3">
                                                                    <div>
                                                                        ...
                                                                        <div>
                                                                            <h5 class="font-size-14 text-truncate"><a href="detail_dossier_export.php?ref_dos='.$row['id_dossier'].'" class="text-body">'.$row['num_dossier'].'</a></h5>
                                                                            <div class="mt-3">
