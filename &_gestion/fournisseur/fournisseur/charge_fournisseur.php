 ...
 if(isset($_POST['recher_pers']) && $_POST['recher_pers']!='')
  ...
  <?php 
    ...
    $_SESSION["recher_pers"]=$recher_pers;
	 ...
	 $requete="SELECT * FROM fournisseur WHERE valide=0 ";
	 
	 if($recher_pers!=""){
	 $requete.=" AND id_fournisseur=".$recher_pers." ";
	 }
	 ...
	 $sqlQuery 	= $con->query("SELECT * FROM fournisseur WHERE valide=0 ");
	 }
  ...
  if($last_page > 1){
   ...
   if($recher_pers!='')
    ...
    $_SESSION["recher_pers"]=$recher_pers;
	 ...
	 $requete.=" ORDER BY code_fournisseur DESC LIMIT $start, $records_per_page";
	...
	$records = $con->query("SELECT * FROM fournisseur WHERE id_fournisseur!='' AND valide='0' ORDER BY code_fournisseur DESC LIMIT $start, $records_per_page");
	 }
...
$HTML.='<p class="col_titre">Nombre de fournisseurs : <b>'.$count_1.'</b>&nbsp;&nbsp;';

$HTML.='
    <a href="fournisseur/exportation/pdf/pdf_fournisseur.php" target="_blank" style="font-size:20px; font-weight:600; color: #da0909" title="Générer le fichier pdf">
        <i class="fa fa-file-pdf"></i>
    </a>  &nbsp;&nbsp;&nbsp;
    <a href="fournisseur/exportation/excel/excel_fournisseur.php" target="_blank" style="font-size:20px; font-weight:600; color: #006f38" title="Générer le fichier excel">
...
$HTML.='	
			<tr>
			<td><span class="c_0">'.stripslashes(strtoupper($row['code_fournisseur'])).'</span></td>
			<td><span class="text-lg text-semibold text-main fo_per">'.stripslashes($row['nom_fournisseur']).'</span></td>
            <td><span class="text-lg text-semibold text-main fo_per">'.stripslashes($row['email_fournisseur']).'</span></td>
            <td><span class="text-lg text-semibold text-main fo_per">'.stripslashes($row['tel_fournisseur']).'</span></td>
            <td><span class="text-lg text-semibold text-main fo_per">'.stripslashes($row['adresse']).'</span></td>
';
			
           $HTML.=' 
        <td>	           
            <a href="javascript:void();" class="btn btn-sm  supr" title="Supprimer" data-toggle="modal" data-target="#modal_sup" data-id='.$row['id_fournisseur'].'><i class="fa fa-trash"></i></a>
            <a href="javascript:void();" class="btn btn-sm  detail" title="D&eacute;tail" data-toggle="modal" data-target="#modal_detail" data-id='.$row['id_fournisseur'].'><i class="fa fa-align-justify fa_jus"></i></a>
		</td>
		</tr>
		';
...
{
    $titre='<br />Aucun fournisseur trouv&eacute;';
    $donnee=$titre;
