...
$sqlQuery = $con->query("SELECT * FROM fournisseur WHERE matricule_fournisseur='".$matr."' AND id_fournisseur!='".$_SESSION['id_pers_soi_mod']."'");
...
$rek=$con->prepare("UPDATE fournisseur SET photo_fournisseur=:A WHERE id_fournisseur=:J");
$rek->execute(array('A'=>$img, 'J'=>$_SESSION['id_pers_soi_mod']));

}

$rep=$con->prepare("UPDATE fournisseur SET matricule_fournisseur=:A, nom_fournisseur=:B, sexe_fournisseur=:C, date_nais_fournisseur=:D, tel_fournisseur=:E, email_fournisseur=:F, service_id=:G, fonction_id=:H, statut_fournisseur_id=:I, date_recrutement=:J WHERE id_fournisseur=:K");
...
$lib_trace="Modification fournisseur (".$matr.") ".$nom_p." ayant pour fonction ".stripslashes($utif['lib_fonction']).' et pour statut '.stripslashes($utis['lib_statut_pers_soignant'])." du service ".$utig['lib_service'];
