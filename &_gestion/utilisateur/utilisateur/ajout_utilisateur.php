...
$inf = $con->query("SELECT * FROM personnel WHERE id_personnel='".$personnel_id."' ");
$info=$inf->fetch();
$nom_utilisateur=$info['nom_personnel'];
$photo_util=$info['photo_personnel'];
$email_utilisateur=$info['email_personnel'];
$tel_utilisateur=$info['tel_personnel']; 
...
{
$req=$con->prepare("INSERT INTO utilisateur(type_groupe_id, motpass_utilisateur, date_creat, secur_ajout, secur, nom_utilisateur, email_utilisateur, tel_utilisateur, photo_util, personnel_id) VALUES (:A, :B, :C, :D, :E, :F, :G, :H, :I, :J)");
$req->execute(array('A'=>$groupe_uti, 'B'=>$pwd, 'C'=>$date, 'D'=>$_SESSION['secur_site'], 'E'=>$securite, 'F'=>$nom_utilisateur, 'G'=>$email_utilisateur, 'H'=>$tel_utilisateur, 'I'=>$photo_util, 'J'=>$personnel_id));

//Envoi du mail 
// on génère une frontière
$destinataire=$email_utilisateur;
$sujet= "CREATION DE COMPTE | STTCI";
...
$message .= " Votre compte utilisateur STTCI a &eacute;t&eacute; cr&eacute;&eacute;. Veuillez trouver ci-dessous vos param&agrave;tres de connexion. <p>Cliquez sur <a style='color:green;' href='app.stt.ci'>>> ce lien <<</a> pour vous connecter : </p><br/><br/>";

$message .= "Parametres de connexion : <br><br> Identifiant : <b>".$email_utilisateur."</b> <br/>Mot de passe : <b>".$pass."</b> <br/>";
$message .= "<br><br><br> <hr> <br><p><a href='www.stt.ci'>STTCI</a> vous remercie.</p> ";
$message .= "\n\n";
...
$req->execute(array('A'=>$lib_trace, 'B'=>$date, 'C'=>$adresse, 'D'=>$_SESSION['secur_site']));	
