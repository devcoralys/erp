...
$sqlQ=$con->prepare("SELECT * FROM utilisateur LEFT JOIN personnel ON personnel.id_personnel=utilisateur.personnel_id INNER JOIN service ON service.id_service=personnel.service_id INNER JOIN fonction ON fonction.id_fonction=personnel.fonction_id INNER JOIN groupe_utilisateur ON groupe_utilisateur.id_type_groupe=utilisateur.type_groupe_id WHERE id_utilisateur=:A");
...
$red->execute(array('A'=>$lib_trace, 'B'=>$date_histo, 'C'=>$adresse, 'D'=>$_SESSION['secur_site']));
