...
$tel_utilisateur=$_POST['tel_personnel_mod']; 
...
$req=$con->prepare(" UPDATE utilisateur SET type_groupe_id=:A, date_mod=:B, secur_mod=:C, statut=:D, personnel_id=:E, nom_utilisateur=:F, email_utilisateur=:G, tel_utilisateur=:H, connecte=:I, valide_util=:K, fonction_id=:L, service_id=:M, photo_util=:N  WHERE id_utilisateur=:O ");
$req->execute(array('A'=>$groupe_uti, 'B'=>$date, 'C'=>$_SESSION['secur'], 'D'=>$statut, 'E'=>$personnel_id, 'F'=>$nom_utilisateur, 'G'=>$email_utilisateur, 'H'=>$tel_utilisateur, 'I'=>$connecte, 'K'=>$valide, 'L'=>$fonction_id, 'M'=>$service_id, 'N'=>$photo_util, 'O'=>$id_utilisateur));
