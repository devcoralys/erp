...
function Footer()
{
   ...
   // $this->Image('../../../img/logo_veritas.jpg', 10,275,30);	
    ...
    $this->Cell(0,10,'Page '.$this->PageNo().'/{nb}',0,0,'C');
			...
			$pdffilename = 'liste_autre_envoi_france.pdf';
			clearstatcache();
    ...
    if(isset($_SESSION['recher_code_colis_autre_envoi_france']) && $_SESSION['recher_code_colis_autre_envoi_france']!='')
	{
	$recher_code_colis_autre_envoi_france=$_SESSION['recher_code_colis_autre_envoi_france'];
	}
	else
	{
...
$nco1=$con->prepare(" SELECT * FROM autre_envoi_france LEFT JOIN colis_autre_envoi_france ON autre_envoi_france.time_autre_envoi_france=colis_autre_envoi_france.autre_envoi_france_time WHERE id_autre_envoi_france='".$row['id_autre_envoi_france']."' AND annulation!=2 AND secur_ajout_autre_envoi_france='".$_SESSION['secur_site']."' ");
    if($_SESSION['acces_tout_autre_envoi_france']==1)
    {
        $nco1=$con->prepare(" SELECT * FROM autre_envoi_france LEFT JOIN colis_autre_envoi_france ON autre_envoi_france.time_autre_envoi_france=colis_autre_envoi_france.autre_envoi_france_time WHERE id_autre_envoi_france='".$row['id_autre_envoi_france']."' AND annulation!=2 ");
    }
$nco1->execute();
...
$tot=$con->prepare('SELECT * FROM autre_envoi_france LEFT JOIN colis_autre_envoi_france ON autre_envoi_france.time_autre_envoi_france=colis_autre_envoi_france.autre_envoi_france_time WHERE id_autre_envoi_france="'.$row['id_autre_envoi_france'].'" AND annulation!=2 AND secur_ajout_autre_envoi_france="'.$_SESSION['secur_site'].'" ');
   if($_SESSION['acces_tout_autre_envoi_france']==1)
    {
        $tot=$con->prepare('SELECT * FROM autre_envoi_france LEFT JOIN colis_autre_envoi_france ON autre_envoi_france.time_autre_envoi_france=colis_autre_envoi_france.autre_envoi_france_time WHERE id_autre_envoi_france="'.$row['id_autre_envoi_france'].'" AND annulation!=2 ');
    }
$tot->execute();
while($itot=$tot->fetch())
{
	$montant_total=$montant_total + ($itot['poids_colis_autre_envoi_france']*$itot['pu_colis_autre_envoi_france']) + $itot['emballage_colis_autre_envoi_france'] + $itot['montant_assurance'];
...
$poi=$con->prepare('SELECT * FROM autre_envoi_france LEFT JOIN colis_autre_envoi_france ON autre_envoi_france.time_autre_envoi_france=colis_autre_envoi_france.autre_envoi_france_time WHERE id_autre_envoi_france="'.$row['id_autre_envoi_france'].'" AND annulation!=2 AND secur_ajout_autre_envoi_france="'.$_SESSION['secur_site'].'" ');
   if($_SESSION['acces_tout_autre_envoi_france']==1)
    {
        $poi=$con->prepare('SELECT * FROM autre_envoi_france LEFT JOIN colis_autre_envoi_france ON autre_envoi_france.time_autre_envoi_france=colis_autre_envoi_france.autre_envoi_france_time WHERE id_autre_envoi_france="'.$row['id_autre_envoi_france'].'" AND annulation!=2 ');
    }
$poi->execute();
while($ipoi=$poi->fetch())
{
	$poids_autre_envoi_france=$poids_autre_envoi_france + $ipoi['poids_colis_autre_envoi_france'];
	
	$nature_colis_autre_envoi_france.=' '.$ipoi['nature_colis_autre_envoi_france'].' ';
}
...
$tot_poids=$tot_poids+$poids_autre_envoi_france;
$tot_nb_col=$tot_nb_col+$nbre_colis_autre_envoi_france;
...
$pdf->SetFont('Arial','B',8);
$pdf->Cell($width_cell[1],7,utf8_decode($row['num_autre_envoi_france']),1,0,'C',false);
$pdf->SetFont('Arial','',8);
$pdf->Cell($width_cell[2],7,utf8_decode(substr($row['destinataire_autre_envoi_france'], 0, 17)),1,0,'L',false);
$pdf->Cell($width_cell[3],7,utf8_decode(substr($row['destination_autre_envoi_france'], 0, 12)),1,0,'L',false);
$pdf->Cell($width_cell[4],7,utf8_decode($row['tel_expediteur_autre_envoi_france']),1,0,'L',false);
$pdf->Cell($width_cell[5],7,utf8_decode($poids_autre_envoi_france),1,0,'C',false);
$pdf->Cell($width_cell[6],7,$nbre_colis_autre_envoi_france,1,0,'C',false);
$pdf->Cell($width_cell[7],7,substr($nature_colis_autre_envoi_france, 0, 21).'...',1,0,'C',false);
