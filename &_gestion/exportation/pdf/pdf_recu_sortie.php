...
if(isset($_GET['id_decaissement']) && $_GET['id_decaissement']!=''){ $_SESSION['id_decaissement']=$_GET['id_decaissement']; }
            
			//Paramètres du fichier PDF 
			$pdffilename = 'recu_decaissement.pdf';
			clearstatcache();
			...
			$pdf->Image("../../../img/entete_fiche_caisse_1_sortie.jpg", 5, 10,  200, 'C');
			$pdf->Ln(25);
	...
	$req1=$con->prepare(" SELECT * FROM decaissement WHERE id_decaissement='".$_SESSION['id_decaissement']."'  ");
	$req1->execute();
	...
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(0,3.3,utf8_decode('N° '.$info_fiche['num_decaissement']),0,1,'L');
	$pdf->Ln(2);
...
/*
	...
	*/
	...
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(0,3.3,utf8_decode('N° Réçu Physique : '.$info_fiche['num_recu_decaissement']),0,1,'L');
	$pdf->Ln(2);
	$pdf->SetFont('Arial','',9);
	$pdf->Cell(0,3.7,utf8_decode('DATE : '.date("d/m/Y", strtotime($info_fiche['date_decaissement'])).' '),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('N° du Dossier : '.$info_fiche['num_dossier_decaissement'].''),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('CLIENT : '.$info_fiche['nom_client_decaissement'].' '),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('Montant : '.number_format($info_fiche['montant_decaissement'],0,',',' ').' FCFA '),0,1,'L');
	$pdf->Ln(2);  
	$pdf->Cell(0,3.7,utf8_decode('Motif : '.$info_fiche['motif_decaissement'].' '),0,1,'L');
	$pdf->Ln(2);  
