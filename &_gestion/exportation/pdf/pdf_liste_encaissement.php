...
function Footer()
{
   ...
   // $this->Image('../../../img/logo_veritas.jpg', 10,275,30);	
    ...
    $this->Cell(0,10,'Page '.$this->PageNo().'/{nb}',0,0,'C');
			...
			$pdffilename = 'liste_encaissement.pdf';
			clearstatcache();
			...
			$pdf->Image("../../../img/entete_encaissement.jpg", 10, 10,  270, 'C');
			$pdf->Ln(25);
    ...
    if($recher_num_recu!=''){
	    $req.=" AND num_encaissement LIKE '%".$recher_num_recu."%' ";
	}
	...
	$req.=" ORDER BY date_encaissement DESC ";
...
/*
	...
	$pdf->Cell(0,3.3,utf8_decode('LISTE DES encaissementS'),0,1,'C');
	$pdf->Ln(4);
*/
...
$montant_total=$montant_total+floatval($row['montant_encaissement']);

 
$pdf->SetTextColor(0, 0, 0);
$pdf->SetFont('Arial','',10);

$pdf->Cell($width_cell[0],7,utf8_decode(date("d/m/Y", strtotime($row['date_encaissement']))),1,0,'C',false);
$pdf->SetFont('Arial','B',12);
$pdf->Cell($width_cell[1],7,utf8_decode($row['num_recu_encaissement']),1,0,'C',false);
$pdf->SetFont('Arial','',10);
$pdf->Cell($width_cell[2],7,utf8_decode($row['num_dossier_encaissement']),1,0,'C',false);
$pdf->Cell($width_cell[3],7,utf8_decode($row['nom_client_encaissement']),1,0,'C',false);
$pdf->Cell($width_cell[4],7,utf8_decode(number_format($row['montant_encaissement'],0,',',' ').' '),1,1,'C',false);
$fill = !$fill;
}
