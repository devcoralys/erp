...
if(isset($_GET['id_encaissement']) && $_GET['id_encaissement']!=''){ $_SESSION['id_encaissement']=$_GET['id_encaissement']; }
            
			//Paramètres du fichier PDF 
			$pdffilename = 'recu_encaissement.pdf';
			clearstatcache();
	...
	$req1=$con->prepare(" SELECT * FROM encaissement WHERE id_encaissement='".$_SESSION['id_encaissement']."'  ");
	$req1->execute();
	...
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(0,3.3,utf8_decode('N° '.$info_fiche['num_encaissement']),0,1,'L');
	$pdf->Ln(2);
...
/*
	...
	*/
	...
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(0,3.3,utf8_decode('N° Réçu Physique : '.$info_fiche['num_recu_encaissement']),0,1,'L');
	$pdf->Ln(2);
	$pdf->SetFont('Arial','',9);
	$pdf->Cell(0,3.7,utf8_decode('DATE : '.date("d/m/Y", strtotime($info_fiche['date_encaissement'])).' '),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('N° du Dossier : '.$info_fiche['num_dossier_encaissement'].''),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('CLIENT : '.$info_fiche['nom_client_encaissement'].' '),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('Montant : '.number_format($info_fiche['montant_encaissement'],0,',',' ').' FCFA        Mode de règlement : '.$info_fiche['mode_reglement_encaissement'].' '),0,1,'L');
	$pdf->Ln(2);  
	$pdf->Cell(0,3.7,utf8_decode('Motif : '.$info_fiche['motif_encaissement'].' '),0,1,'L');
	$pdf->Ln(2);  
...
$pdf->Cell(0,4,utf8_decode('La Comptabilité (date et visa)                                                                                                  Le Client (date et visa)'),0,1,'R'); 
$pdf->Ln(40);
