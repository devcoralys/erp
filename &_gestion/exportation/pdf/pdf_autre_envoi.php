...
if(isset($_GET['id_autre_envoi']) && $_GET['id_autre_envoi']!=''){ $_SESSION['id_autre_envoi']=$_GET['id_autre_envoi']; }
            
			//Paramètres du fichier PDF 
			$pdffilename = 'fiche_autre_envoi.pdf';
			clearstatcache();
	...
	$req=" SELECT * FROM autre_envoi LEFT JOIN colis_autre_envoi ON autre_envoi.time_autre_envoi=colis_autre_envoi.autre_envoi_time WHERE id_autre_envoi='".$_SESSION['id_autre_envoi']."'  ";
	...
	$req0=$con->prepare(" SELECT * FROM autre_envoi LEFT JOIN colis_autre_envoi ON autre_envoi.time_autre_envoi=colis_autre_envoi.autre_envoi_time LEFT JOIN utilisateur ON utilisateur.secur=autre_envoi.secur_ajout_autre_envoi LEFT JOIN personnel ON personnel.id_personnel=utilisateur.personnel_id WHERE id_autre_envoi='".$_SESSION['id_autre_envoi']."'  ");
	$req0->execute();
    ...
    $pdf->SetFont('Arial','',8);
    $pdf->Ln(2);
    $pdf->Cell(0,4,utf8_decode(strtoupper(' Vous ne disposez que de trois jours pour la recuperation de votre colis_autre_envoi a compter de la date de notification.')),0,1,'C'); 
     $pdf->Cell(0,4,utf8_decode(strtoupper('Passe ce delai, nous declinons toute responsabilite vis a vis du colis ')),0,1,'C'); 
    $pdf->SetTextColor(0, 0, 0);
		*/
		    $pdf->Ln(5);	
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(0,3.3,utf8_decode('Ref: '.$info_fiche0['num_autre_envoi'].' | '.$info_fiche0['nom_utilisateur'].' | '.$info_fiche0['tel_personnel']),0,1,'L');
	$pdf->Ln(2);
	...
	$req1=$con->prepare(" SELECT * FROM autre_envoi LEFT JOIN colis_autre_envoi ON autre_envoi.time_autre_envoi=colis_autre_envoi.autre_envoi_time WHERE id_autre_envoi='".$_SESSION['id_autre_envoi']."'  ");
	$req1->execute();
	$info_fiche=$req1->fetch();	


	$nbre_colis_autre_envoi=0;
	$nco1=$con->prepare(" SELECT * FROM autre_envoi LEFT JOIN colis_autre_envoi ON autre_envoi.time_autre_envoi=colis_autre_envoi.autre_envoi_time WHERE id_autre_envoi='".$_SESSION['id_autre_envoi']."'  ");
	$nco1->execute();
	...
	$pdf->Cell(0,4,$info_fiche['mode_envoi'].' / '.$info_fiche['num_autre_envoi'],0,0,'R'); 	
	$pdf->Ln(2);
	...
	$pdf->Cell(-153,3.3,utf8_decode($nbre_colis_autre_envoi),0,0,'C');
	$pdf->Ln(4);
	...
	$pdf->SetFont('Arial','',9);
	$pdf->Cell(0,3.7,utf8_decode('DATE : '.date("d/m/Y", strtotime($info_fiche['date_autre_envoi'])).' '.date("H:i:s", strtotime($info_fiche['date_creat_autre_envoi'])).' '),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('EXPEDITEUR : '.$info_fiche['expediteur_autre_envoi'].'                                                                                             DESTINATION : '.$info_fiche['destination_autre_envoi'].' '),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('N° CNI EXPEDITEUR : '.$info_fiche['num_cni_expediteur_autre_envoi'].'                                                                                                 DESTINATAIRE : '.$info_fiche['destinataire_autre_envoi'].' '),0,1,'L');
	$pdf->Ln(2);
	$pdf->Cell(0,3.7,utf8_decode('TEL : '.$info_fiche['tel_expediteur_autre_envoi'].'                                                                                                                       TEL : '.$info_fiche['tel_destinataire_autre_envoi'].' '),0,1,'L');
	$pdf->Ln(2);     
...
$pdf->Cell($width_cell[1],7,utf8_decode($row['nbre_colis_autre_envoi']),1,0,'C',false);
$pdf->Cell($width_cell[2],7,utf8_decode($row['nbre_article']),1,0,'C',false);
$pdf->Cell($width_cell[3],7,utf8_decode($row['nature_colis_autre_envoi']),1,0,'C',false);
$pdf->Cell($width_cell[4],7,utf8_decode($row['poids_colis_autre_envoi']),1,0,'C',false);
$pdf->Cell($width_cell[5],7,utf8_decode(number_format($row['pu_colis_autre_envoi'],0,',',' ').' FCFA'),1,0,'C',false);
$pdf->Cell($width_cell[6],7,utf8_decode(number_format($row['poids_colis_autre_envoi']*$row['pu_colis_autre_envoi'],0,',',' ').' FCFA'),1,1,'C',false);
$fill = !$fill;
}
...
while($itot=$tot->fetch())
{
	...
	$montant_total=$montant_total + ($itot['poids_colis_autre_envoi']*$itot['pu_colis_autre_envoi']) + $itot['emballage_colis_autre_envoi'];//+$montant_assurance;
...
$emb=$con->prepare('SELECT * FROM autre_envoi LEFT JOIN colis_autre_envoi ON autre_envoi.time_autre_envoi=colis_autre_envoi.autre_envoi_time WHERE id_autre_envoi="'.$_SESSION['id_autre_envoi'].'"  ');
$emb->execute();
$montant_embal=0;
while($iemb=$emb->fetch())
{
	$montant_embal=$montant_embal + $iemb['emballage_colis_autre_envoi'];
}

//Sous-Total
$sous=$con->prepare('SELECT * FROM autre_envoi LEFT JOIN colis_autre_envoi ON autre_envoi.time_autre_envoi=colis_autre_envoi.autre_envoi_time WHERE id_autre_envoi="'.$_SESSION['id_autre_envoi'].'"  ');
$sous->execute();
$montant_sousal=0;
while($isous=$sous->fetch())
{
	$montant_sousal=$montant_sousal + ($isous['poids_colis_autre_envoi']*$isous['pu_colis_autre_envoi']) ;
}

//Calcul Total Assurance
/*
$ass=$con->prepare('SELECT * FROM autre_envoi LEFT JOIN colis_autre_envoi ON autre_envoi.time_autre_envoi=colis_autre_envoi.autre_envoi_time WHERE id_autre_envoi="'.$_SESSION['id_autre_envoi'].'"  ');
$ass->execute();
$montant_assurance=0;
...
$pdf->SetFont('Arial','B',8);
$pdf->Cell(0,4,utf8_decode('Adresse de recupération : '.$info_fiche['adresse_recuperation_autre_envoi']),0,1,'L'); 
$pdf->Ln(1);
