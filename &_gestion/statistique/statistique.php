...
//Dépenses
$dep=$con->prepare('SELECT * FROM depense ');
$dep->execute();
while($idep=$dep->fetch())
{
    $ctr=$con->prepare('SELECT * FROM mouvement WHERE num_mouvement=:A');
    $ctr->execute(array('A'=>$idep['num_depense']));
    $nc=$ctr->rowcount();
    if($nc<=0)
    {
    $ins=$con->prepare('INSERT INTO mouvement(date_creat_mouvement, secur_ajout_mouvement, type_mouvement, montant_mouvement, num_mouvement, date_mouvement) VALUES (:A, :B, :C, :D, :E, :F)');
    $ins->execute(array('A'=>$idep['date_creat_depense'], 'B'=>$idep['secur_ajout_depense'], 'C'=>0, 'D'=>$idep['montant_depense'], 'E'=>$idep['num_depense'], 'F'=>$idep['date_creat_depense']));
    }
}

//Entrées
/*
$cai=$con->prepare('SELECT * FROM caisse ');
$cai->execute();
while($icai=$cai->fetch())
{
    $ctr=$con->prepare('SELECT * FROM mouvement WHERE num_mouvement=:A');
    $ctr->execute(array('A'=>$icai['num_caisse']));
    $nc=$ctr->rowcount();
    if($nc<=0)
    {
    $ins=$con->prepare('INSERT INTO mouvement(date_creat_mouvement, secur_ajout_mouvement, type_mouvement, montant_mouvement, num_mouvement, date_mouvement) VALUES (:A, :B, :C, :D, :E, :F)');
    $ins->execute(array('A'=>$icai['date_creat_caisse'], 'B'=>$icai['secur_ajout_caisse'], 'C'=>1, 'D'=>$icai['montant_caisse'], 'E'=>$icai['num_caisse'], 'F'=>$icai['date_creat_caisse']));
    }
}
*/
...
<html lang="en">
    ...
    <head>
        ...
        <meta charset="utf-8" />
        <title><?php include('../titre_ent_1.php'); ?> | Statistique</title>
...
<!-- Left Sidebar End -->
            ...
            <div class="page-content">
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-flex align-items-center justify-content-between">
                              ...
                              <!--  <button id="ajout_caisse" data-toggle="modal" data-target="#myModal_caisse" style="margin:10px; " class="btn btn-purple"><i class="fa fa-plus"></i> Créer un ticket</button>
                            -->
                            </div>
                            <!--
                            <a href="depense.php"><button style="margin:10px; " class="btn btn-danger"><i class="fa fa-">-</i> Enregistrer une dépense</button></a>
                            -->
                        </div>
                    </div>
                    <!-- end page title -->
                    ...
                    <div class="col-lg-6 col-xs-12" style="margin-top:10px;">
                        <label class="label_col">Date de debut</label>
                        <input type="date" class="form-control"  id="recher_date_debut" name="recher_date_debut"/>
		            </div>
                    <div class="col-lg-6 col-xs-12" style="margin-top:10px;">
                        <label class="label_col">Date de fin</label>
                        <input type="date" class="form-control"  id="recher_date_fin" name="recher_date_fin"/>
		            </div>
    ...
    <a href="excel/excel_mouvement.php" target="_blank" title="Exporter en Excel" id="title">
        <i class="fa fa-file-excel fa-2x" style="color:green;"></i>
                        ...
                        <div class="page-content-wrap"  style="">
                            ...
                            <div class="row voir aff_statistique">
