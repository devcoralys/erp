...
$sqlQuery = $con->query("SELECT * FROM client WHERE matricule_client='".$matr."' AND id_client!='".$_SESSION['id_pers_soi_mod']."'");
...
$rek=$con->prepare("UPDATE client SET photo_client=:A WHERE id_client=:J");
$rek->execute(array('A'=>$img, 'J'=>$_SESSION['id_pers_soi_mod']));

}

$rep=$con->prepare("UPDATE client SET matricule_client=:A, nom_client=:B, sexe_client=:C, date_nais_client=:D, tel_client=:E, email_client=:F, service_id=:G, fonction_id=:H, statut_client_id=:I, date_recrutement=:J WHERE id_client=:K");
...
$lib_trace="Modification client (".$matr.") ".$nom_p." ayant pour fonction ".stripslashes($utif['lib_fonction']).' et pour statut '.stripslashes($utis['lib_statut_pers_soignant'])." du service ".$utig['lib_service'];
