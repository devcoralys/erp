...
function Footer()
{
    ...
    //$this->Cell(0,10,'Page '.$this->PageNo().'/{nb}',0,0,'C');
			...
			$pdffilename = 'liste_client_sttci.pdf';
			clearstatcache();
...
$pdf->Cell(0,3.3,'LISTE DES CLIENTS',0,1,'C');
...
$pdf->Cell(0,3.7,utf8_decode('Nombre de clients trouvés : '.$nbre_pers.' '),0,1,'L');
...
$pdf->Cell($width_cell[1],7,utf8_decode('NOM DU CLIENT'),1,0,'C',true);
...
$pdf->Cell($width_cell[0],7,utf8_decode($row['code_client']),1,0,'C',true);
$pdf->Cell($width_cell[1],7,utf8_decode($row['nom_client']),1,0,'C',true);
$pdf->Cell($width_cell[2],7,utf8_decode($row['email_client']),1,0,'C',true);
$pdf->Cell($width_cell[3],7,utf8_decode($row['tel_client']),1,0,'C',true);
