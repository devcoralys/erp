...
$date_nais = $row['date_nais_client'];
$date_recru = $row['date_recrutement'];
$_SESSION['id_pers_soi_mod']=$row['id_client'];
?>
			                            ...
			                            <div class="col_pho" id="image_preview_mod">
											<?php  if($row['photo_client']!='') { ?>
							<img src="../../photo/<?php echo $row['photo_client']; ?>" width="147px" height="150px" alt="Photo de profil">
											<?php }else{ ?>
										...
										<!-- <div class="form-group">
					                            ...
					                            <div class="col-lg-8">
					                                <div class="radio">
 <input id="mascu_mod" class="magic-radio" type="radio" name="sexe_mod" value="1" <?php //if($row['sexe_client']=="1"){ echo "checked"; } ?>>
					                                    <label for="mascu_mod">Masculin</label>
					
 <input id="fem_mod" class="magic-radio" type="radio" name="sexe_mod" value="2" <?php //if($row['sexe_client']=="2"){ echo "checked"; } ?>>
...
<input type="text" class="form-control" name="mat_mod" id="mat_mod" placeholder="Matricule" value="<?php echo $row['matricule_client']; ?>">
					                            </div>
					                        </div>
					                        <div class="form-group">
					                            <label class="col-lg-4 control-label">Nom & pr&eacute;nom(s)<span class="rouge">*</span></label>
					                            <div class="col-lg-8">
<input type="text" class="form-control" name="nom_mod" id="nom_mod" placeholder="Nom & pr&eacute;nom(s)" value="<?php echo $row['nom_client']; ?>">
					                            </div>
					                        </div>
											<div class="form-group">
					                            <label class="col-lg-4 control-label"  style="margin-top:10px">Sexe<span class="rouge">*</span></label>
					                            <div class="col-lg-8">
					                                <div class="radio">
 <input id="mascu_mod" class="magic-radio" type="radio" name="sexe_mod" value="M" <?php if($row['sexe_client']=="M"){ echo "checked"; } ?>>
					                                    <label for="mascu_mod">Masculin</label>
					
 <input id="fem_mod" class="magic-radio" type="radio" name="sexe_mod" value="F" <?php if($row['sexe_client']=="F"){ echo "checked"; } ?>>
					                                    <label for="fem_mod">F&eacute;minin</label>
					                                </div>
					                            </div>
					                        </div>
...
<input type="date" id="date_nais_mod" name="date_nais_mod" class="form-control" value="<?php echo $date_nais; ?>">
			...
			<input type="text" class="form-control" name="tel_mod" id="tel_mod" placeholder="T&eacute;l&eacute;phone" value="<?php echo $row['tel_client']; ?>">
					                            </div>
					                        </div>
											<div class="form-group">
					                            <label class="col-lg-4 control-label">Email<span class="rouge">*</span></label>
					                            <div class="col-lg-8" style="margin-bottom:10px">
<input type="email" class="form-control" name="email_mod" id="email_mod" placeholder="Email" value="<?php echo $row['email_client']; ?>">
					                            </div>
					                        </div>
											<div class="form-group">
 ...
 <select data-trigger class="form-control fonctio_mod" id="demo-chosen-select_" title="Choisir une fonction..." name="fonctio_mod" data-live-search="true" data-width="100%">
	...
	<select data-trigger class="form-control statut_mod" id="demo-chosen-select_1" title="Choisir un statut..." name="statut_mod" data-live-search="true" data-width="100%">
						                            <option value="">Choisir un statut...</option>
					                                  <?php
	                                               $rem_3=$con->prepare("SELECT * FROM statut_client ORDER BY lib_statut_client ASC");
                                                     $rem_3->execute();
                                                     while($rom_3=$rem_3->fetch())
	                                                 {
	                                                 ?>
  <option value="<?php echo''.$rom_3['id_statut_client'].''; ?>" <?php if($rom_3['id_statut_client']==$row['statut_client_id']){ echo'selected'; } ?>><?php echo''.stripslashes($rom_3['lib_statut_client']).''; ?></option>
                                                     <?php
	                                                 }
	                                                 ?>
					                            </select>
