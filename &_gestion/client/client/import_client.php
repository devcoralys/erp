...
$red=$con->prepare("SELECT * FROM client WHERE email_client =:A OR tel_client=:B "); 
...
if ($red->rowCount())
{
  ...
  $lib_doublon='Doublon détecté sur le client <b>'.$nom_p.'</b> Veuillez vérifier votre base de données';
...
else
{
  ...
  if($nom_p!='' && $tel!='' && $email!='')
  {
    ...
    $code_client='STT'.gmdate('Ymd').'-000'.$nb;
    ...
    $req->execute(array('A'=>$code_client, 'B'=>$nom_p, 'E'=>$tel, 'F'=>$email, 'K'=>$date, 'L'=>$_SESSION['secur_site'], 'M'=>$adresse));
  }
}
...
$date=gmdate('Y-m-d H:i:s');
  ...
  $lib_trace="Ajout du client <b>".$nom_p."</b> | email <b>".$email."</b> | telephone : <b>".$tel."</b> | adresse : <b>".$adresse."</b>";
