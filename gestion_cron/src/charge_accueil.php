<?php
  ...
  <?php 
    ...
    $_SESSION["recher_affectation"]=$recher_affectation;
	 ...
	 $requete="SELECT * FROM fiche WHERE id_fiche!=''AND etat_fiche=0 AND decaisse=0 AND sauvegarder=0 AND approuve=1 ";
	...
	*/
	 $sqlQuery 	= $con->query("SELECT * FROM fiche WHERE id_fiche!='' AND etat_fiche=0 AND decaisse=0 AND sauvegarder=0 AND approuve=1 ");
	 }
  ...
  if($last_page > 1){
   ...
   if($recher_date_debut!='' || $recher_date_fin!='' || $recher_demandeur!='' || $recher_chantier!='' || $recher_affectation!='')
	...
	$records = $con->query("SELECT * FROM fiche WHERE id_fiche!='' AND etat_fiche=0 AND decaisse=0 AND sauvegarder=0 AND approuve=1 ORDER BY id_fiche DESC LIMIT $start, $records_per_page");
	 }
