...
//Mise à jour des décaissements effectués à ce jour
/*
$ls=$con->prepare('SELECT * FROM fiche WHERE decaisse=1 AND secur_decaisse!="" ');
$ls->execute();
while($ils=$ls->fetch())
{
    $ver=$con->prepare('SELECT * FROM decaissement WHERE num_fiche_decaissement=:A');
    $ver->execute(array('A'=>$ils['num_fiche']));
    $nv=$ver->rowcount();
    if($nv<=0)
    {
        $adec=$con->prepare('INSERT INTO decaissement(num_fiche_decaissement, montant, secur_decaissement) VALUES(:A, :B, :C)');
        $adec->execute(array('A'=>$ils['num_fiche'], 'B'=>$ils['montant_fiche'], 'C'=>$ils['secur_decaisse']));
    }
    else
    {
          //tra�abilite
    ...
    $lib_trace="Décaissement double échoué sur la fiche <b>".$ils['num_fiche']."</b> ";
            ...
            echo '
                                            ...
                                            <div class="card border shadow-none">
                                                ...
                                                if($_SESSION['is_modif']==1){
                                                    echo '    
                                                        <a target="_blank" href="confirm_decaisse.php?num_fiche='.$repo['num_fiche'].'" class="btn btn-success"><button type="button" class="btn btn-outline-light" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Profile"><i class="uil uil-sign-out-alt"></i></button></a>
                                                    ';
                                                    }
...
/*
            ...
            <tr>
                ...
                <td>
                    ...
                    <a href="confirm_decaisse.php?num_fiche='.$repo['num_fiche'].'" class="btn btn-success"><i class="fa fa-sign-out"></i> Décaisser</a>
                </td>
            </tr>
            ';
            */
